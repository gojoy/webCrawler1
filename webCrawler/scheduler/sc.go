...
func(m *myScheduler) saveReqToCache(req base.Request,code string) bool  {
	if req.HttpReq()==nil {
		log.Panicln("save req err:nil req")
		return false
	}
	if req.HttpReq().URL==nil {
		log.Println("save req err:nil url")
		return false
	}
	reqUrl:=req.HttpReq().URL

	if  strings.ToLower(reqUrl.Scheme)!="http"{
		log.Println("save req err:url not http")
		return false
	}
	if _,ok:=m.urlMap[reqUrl.String()];ok {
		log.Printf("ingore the req!url is repeated.requrl=%s\n",reqUrl)
		return false
	}
	if pname,_:=getPrimaryDomain(req.HttpReq().Host);pname!=m.primaryDomain {
		log.Printf("ingore the req!host %s is not in primaryDoamin\n",pname)
		return false
	}
	if req.Depth()>m.crawlDepth {
		log.Printf("ingore the req!depth %d is bigger than our depth %d",req.Depth(),m.crawlDepth)
		return false
	}
	if m.stopSign.Signed() {
		m.stopSign.Deal(code)
